...
| `DonggriWorld` | 신규 서비스 시드 | 서비스 아이디어를 제품 요구사항으로 구체화하는 초기 저장소 | README 명세, 구조 설계, 첫 공개 데모 |
...
| 저장소 | 현재 역할 | 서비스 설명 | 다음 개선 포인트 |
| --- | --- | --- | --- |
| `cloudflare-blog` | Cloudflare 블로그 플랫폼 | Public, Admin, API를 분리한 블로그 워크스페이스 | 역할 분리, 미리보기, SEO와 RSS 완성 |
...
- `DonggriWorld`는 서비스 시드 저장소이므로 명세와 첫 데모를 빠르게 고정하는 것이 중요합니다.
- `Favorit`은 오픈소스 UI 라이브러리를 활용해 Windows 위젯 느낌을 강하게 살리면 완성도가 크게 올라갑니다.
- `cloudflare-blog`는 구조가 이미 좋아서 CMS와 운영 UX를 붙이면 제품 레벨에 가까워질 수 있습니다.
